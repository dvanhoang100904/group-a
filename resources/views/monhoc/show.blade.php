...
<div class="py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="fw-bold text-primary">
            <i class="bi bi-journal-bookmark"></i> Chi tiết Môn học
        </h4>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <table class="table table-bordered">
                <tr>
                    <th style="width: 250px;">Mã môn học</th>
                    <td>{{ $monhoc->code }}</td>
                </tr>
                <tr>
                    <th>Tên môn học</th>
                ...
                <tr>
                    <th>Khoa / Bộ môn</th>
                    <td>{{ $monhoc->department->name ?? 'Không xác định' }}</td>
                </tr>
                <tr>
                    <th>Số lượng tài liệu</th>
                    <td>{{ $monhoc->documents ? $monhoc->documents->count() : 0 }}</td>
                </tr>
                ...
                <tr>
                    <th>Ngày tạo</th>
                    <td>{{ $monhoc->created_at->format('d/m/Y H:i') }}</td>
                </tr>
                <tr>
                    <th>Cập nhật lần cuối</th>
                    <td>{{ $monhoc->updated_at->format('d/m/Y H:i') }}</td>
                </tr>
            </table>
            ...
            <div class="mt-3 text-end">
                ...
                <form action="{{ route('monhoc.destroy', $monhoc->subject_id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa môn học này không?')">
